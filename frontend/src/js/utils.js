...
export const emptyFrame = () =>
  Array.from(Array(config.totalPixels)).map((v) => (v = "B"));

export const isEmptyFrame = (frame) => {
  return JSON.stringify(frame) === JSON.stringify(emptyFrame());
};

export const framesEmpty = (frames) => {
  return (
    frames.map((frame) => isEmptyFrame(frame)).filter((f) => !f).length == 0
  );
};
...
export const formatAccountAddress = (account, lsize = 8, rsize = 4) => {
  return (
    account.substring(0, lsize) +
    "..." +
    account.substring(account.length - rsize)
  );
};
...
export const updateInfo = (thingInfo, updates) => {
  Object.keys(updates).forEach((update) => {
    ...
    if (typeof thingInfo[update] !== "undefined")
      thingInfo[update] = updates[update];
  });
  return thingInfo;
...
export const toBigNumberPrecision = (value = null, precision) => {
  ...
  try {
    return toBigNumber(
      stringToFixed(toBigNumber(value).toFixed(precision), precision),
    );
  } catch (e) {
...
export const displayBalanceToPrecision = (value, precision) => {
  if (!value) return "0";
  return toBigNumberPrecision(value, precision).toFormat({
    decimalSeparator: ".",
    groupSeparator: ",",
    groupSize: 3,
  });
...
export const formatLocaleString = (date) =>
